...
{	
	...
	$query_canc_img = "select file from $table where id='$id_canc'";
...
if($azione=="cancella_sel") {
	...
	for($z=0; $z<count($temp)-1; $z++){
		$query_canc_img = "select file from $table where id='".$temp[$z]."'";
		$risu_canc_img = $open_connection->connection->query($query_canc_img);
		if ($risu_canc_img) {
			list($file) = $risu_canc_img->fetch();
			if (is_file("$directory/$file")) @unlink("$directory/$file");
...
<div class="mws-panel grid_8">
	<div style="height:50px;font-size:1.2em;padding-top:10px"><b>BILANCI</b></div>
	...
	<div style="float:right; width:200px; height:30px; text-align:right; margin-bottom:10px;">
		<a href="admin.php?cmd=<?php echo $pagina;?>_ins<?php echo $rif;?>"><button class="btn btn-success"><b>Aggiungi</b></button></a> &nbsp; 
	</div>
