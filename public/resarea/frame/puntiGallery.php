	...
	if($id_rife!=""){
		...
		<!-- Inner Container Start -->
			...
			<div class="container">
				...
				$table=$prec_db."punti_mappa_gallery";
				...
				{	
					...
					if ($risu_canc_img) {
						...
						$temp = explode("/",$img);
						$nome_img = $temp[count($temp)-1];
						$percorso_img = "../img_up/punti/".str_replace($nome_img,'',$img);
						if (is_file("$percorso_img"."s_".$nome_img)) @unlink("$percorso_img"."s_".$nome_img);
						if (is_file("$percorso_img"."xs_".$nome_img)) @unlink("$percorso_img"."xs_".$nome_img);
					}					
				...
				?>
					<script language="javascript">		
						...
						window.location="frame/puntiGallery.php<?php echo $rif;?>&pag_att=<?php echo $pag_att;?>";
					</script>
				<?php 
				} 
				...
				if($azione=="cancella_sel") {
					...
					for($z=0; $z<count($temp)-1; $z++){
						...
						$query_canc_img = "select image from $table where id='".$temp[$z]."'";
						...
						if ($risu_canc_img) {
							...
							$percorso_img = "../img_up/prodotti/".str_replace($nome_img,'',$foto);
						...
						<script type="text/javascript">
							...
							window.location='frame/puntiGallery.php<?php echo $rif;?>';
				...
				<script type="text/javascript">
					...
					function aggiungi_lista(id_check, id_campo){
						...
						if(lista_del!=""){
							...
							document.getElementById('cancella_sel').href='frame/puntiGallery.php<?php echo $rif;?>&pag_att=<?php echo $pag_att;?>&azione=cancella_sel&lista='+lista_del;
					...
					function aggiugni_tutti(){
						...
						if(document.getElementById('check_tutti').checked){
							...
							document.getElementById('cancella_sel').href='frame/puntiGallery.php<?php echo $rif;?>&pag_att=<?php echo $pag_att;?>&azione=cancella_sel&lista='+lista_tutti;
					...
					function verifica(){
						...
						if (document.inserimento.foto.value=="") alert('Foto obbigatoria');	
				...
				if($stato=="inviato")
				{
					...
					for($x=0; $x<count ($_FILES['img']['name']); $x++){
						...
						if ($nome_file) {		
							$nome_file =  $oggetto_admin->scrivi_img($nome_file , $temp_file, "../img_up/punti");
							list($larghezza_gr , $height, $type, $attr) = getimagesize($temp_file);
							if ($larghezza_gr>1920) $oggetto_admin->thumbjpg_new(1920,$temp_file,$nome_file,"../img_up/punti");		
							$oggetto_admin->thumbjpg(400,$temp_file,$nome_file,"../img_up/punti","s_");
							$oggetto_admin->thumbjpg(200,$temp_file,$nome_file,"../img_up/punti","xs_");
							$ordine = $oggetto_admin->trova_ordine($table, "id_rife", "$id_rife");
							//echo $ordine;
							$query_ins_file = "insert into $table (img, id_rife, ordine) values ('$nome_file', $id_rife, $ordine)";
				...
				<div class="mws-panel grid_12">
					<div style="height:50px;font-size:1.2em;padding-top:10px"><b>Foto Prodotto </b></div>					
					...
					<div class="mws-panel-body no-padding" style="display:none" id="addImg">
						...
						<form name="inserimento" class="mws-form" action="frame/puntiGallery.php<?php echo $rif;?>" method="post" enctype="multipart/form-data" onchange="this.submit()">
					...
					?>
							...
							<div class="mws-button-row">
								...
								<?php /*<input type="button" value="Inserisci" class="btn btn-danger" onclick="verifica()">*/?>
					...
					<div class="mws-panel-header" style="margin-top:40px;">
						...
						<span style="color:#DB912D;"><i class="icon-table"></i> Elenco Foto</span>
						...
						<table class="mws-datatable-fn mws-table">
							...
							<thead>
								...
								<tr>
									...
									<th style="width:200px !important">Foto</th>
							...
							<?php 
								...
								{						  
									...
									{						
										...
										$link=$http.'://'.$ind_sito.'/resarea/img_up/punti/'.$foto;
										...
										<tr>
											...
											<td style="position:relative">
												...
												<img src="img_up/punti/<?php if(is_file("img_up/punti/s_".$foto)) echo "s_";?><?php  echo $foto; ?>" style="width:200px"/>
											...
											<td style="width:10%">
												...
												<span class="btn-group">
													...
													<a href="frame/puntiGallery.php<?php echo $rif;?>&id_canc=<?php  echo $id_campo; ?>&azione=primo&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-arrow-up-2"></i></a>

													<a href="frame/puntiGallery.php<?php echo $rif;?>&id_canc=<?php  echo $id_campo; ?>&azione=sali&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-arrow-up"></i></a>

													<a href="frame/puntiGallery.php<?php echo $rif;?>&id_canc=<?php  echo $id_campo; ?>&azione=scendi&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-arrow-down"></i></a>

													<a href="frame/puntiGallery.php<?php echo $rif;?>&id_canc=<?php  echo $id_campo; ?>&azione=ultimo&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-arrow-down-2"></i></a>							

													<a onclick="return confirm('Cancellare questo elemento?')" href="<?php echo $http;?>://<?php echo $ind_sito;?>/resarea/frame/puntiGallery.php<?php echo $rif;?>&azione=cancella&id_canc=<?php  echo $id_campo; ?>&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-trash"></i></a>
