...
{	
	...
	$query_canc_img = "select pdf from $table where id='$id_canc'";
	$risu_canc_img = $open_connection->connection->query($query_canc_img);
	if ($risu_canc_img) {
		list($file) = $risu_canc_img->fetch();
		if (is_file("files/certificazioni/$file")) @unlink("files/certificazioni/$file");
	}
...
<div class="mws-panel grid_8">
	...
	<div style="float:right; width:200px; height:30px; text-align:right; margin-bottom:10px;">
		<a href="admin.php?cmd=<?php echo $table;?>_ins<?php echo $rif;?>"><button class="btn btn-success"><b>Aggiungi <?php echo $table;?></b></button></a> &nbsp; 
	</div>
		...
		<table class="mws-datatable-fn mws-table">
			<thead>
				<tr>
					...
					<th style="text-align:left">Nome File</th>
					<th>Visibile</th>
			...
			<?php 
				...
				{		
					...
					{						
						...
						$pdf = $arr_ele['pdf'];
						$descrizione = $arr_ele['descrizione'];
			...
			?>	
				...
				<tr>
					...
					<td align="left" valign="center">
						<?php  echo $nome; ?>
						<br/>
						<i><?php  echo ucfirst($categoria); ?></i>
					</td>
					<td align="left" valign="center"><?php  echo $descrizione; ?></td>
					<td align="left" valign="center"><?php  echo $pdf; ?></td>
					<td align="center" valign="center">
						<?php   
						if ($visibile==1) {?>
							<a href="admin.php?cmd=<?php echo $table;?>&azione=disattiva&id_canc=<?php echo $id_campo;?><?php echo $rif;?>&pag_att=<?php echo $pag_att;?>" title="Rendi non visibile">
								<i class="fa-solid fa-circle" style="font-size:1.5em; color:green"></i>
							</a>
						<?php }else{?>
							<a href="admin.php?cmd=<?php echo $table;?>&azione=attiva&id_canc=<?php echo $id_campo;?><?php echo $rif;?>&pag_att=<?php echo $pag_att;?>" title="Rendi visibile">
								<i class="fa-regular fa-circle" style="font-size:1.5em; color:#242428"></i>
							</a>
						<?php }?>
					</td>
					<td style="width:10%; text-align:left">
						<span class="btn-group">
							...
							<a  onClick="return confirm('Cancellare gli elementi selezionati?')"href="admin.php?cmd=<?php echo $table;?>&azione=cancella&id_canc=<?php  echo $id_campo; ?><?php echo $rif;?>&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-trash"></i></a>
						</span>
					</td>
				</tr>
			<?php 
