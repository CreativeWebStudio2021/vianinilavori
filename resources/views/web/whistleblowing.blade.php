			...
			<div class="mainTextContainer mainTextContainer2 " style="position:relative; z-index:1;">
				...
				@php
					$query_intro = DB::table('testi_introduttivi')
						->select('testo')
						->where('pagina','=','Whistleblowing')
						->get();
				@endphp
				<div class="expand-block__content" style="margin-top:30px; margin-bottom:60px; border:none !important;">
					{!! str_replace('<a href="https://vianinilavorispa.integrityline.com">','<a target="_blank" href="https://vianinilavorispa.integrityline.com">',html_entity_decode($query_intro[0]->testo)) !!}
				</div>
			</div>
		</section>
						...
						<ul>
							<li style="display: flex; align-items: center;  margin-bottom: 24.2px;">
							  <img class="icon-li" src="{{ asset('web/images/icon_pdf_b.png') }}" alt="Codice Etico - Codice Etico e di Condotta - {{ config('app.name') }}">
								<a href="{{ mostra_pdf_url('Whistleblowing-Informazioni-per-lutenza.pdf', 'Whistleblowing - ' . config('app.name'), 'doc') }}" target="_blank" title="Whistleblowing - {{ config('app.name') }}" target="_blank" class="link-block">
