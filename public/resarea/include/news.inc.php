...
function cambiaNews(id) {
  ...
  document.getElementById('hiddenFrame').src = 'frame/cambiaNews.php?id_campo=' + id;
}
</script>
<div class="mws-panel grid_8">
	...
	<div style="float:left; width:calc(100% - 220px); height:50px;  display:flex; gap:20px">
		<div>
			Categoria<br/>
			<select name="ric_cat" onchange="window.location='admin.php?cmd=news&ric_cat='+this.value">
				<option></option>
				<?php 
				$query_cat="SELECT * FROM categorie_news ORDER BY ordine DESC";
	...
	<div style="float:right; width:200px; height:30px; text-align:right">
		<a href="admin.php?cmd=news_ins<?php echo $rif;?>&pag_att=<?php echo $pag_att;?>"><button class="btn btn-success"><b>Aggiungi articolo</b></button></a> &nbsp; 
	</div>
	...
	<div class="mws-panel-body no-padding">
		<table class="mws-datatable-fn mws-table">
			...
			<tbody>
			<?php 
				$query_ele = "select * from news WHERE 1 $criterio  order by data_news DESC";
			...
			?>	
				...
				<tr>
					...
					<td align="center" valign="center">
						<?php  if ($foto!="" && file_exists("img_up/news/$foto")) { ?>
							<img src="img_up/news/<?php  if (file_exists("img_up/news/s_$foto")) { ?>s_<?php }?><?php  echo $foto; ?>" alt="" style="width:100%"/>
						<?php  } ?>
					</td>
					<td align="left" valign="center">
						...
						<?php if(isset($categoria)){
							$query_c="SELECT nome FROM categorie_news WHERE id='$categoria'";
					...
					<td style="width:10%">
						<span class="btn-group">
							<a title="Visibilità" onclick="cambiaNews('<?php echo $id_campo;?>')" id="news_<?php echo $id_campo;?>" class="btn btn-small" style="cursor:pointer; color:<?php if($visibile==0){?>red<?php }else{?>green<?php }?>"><i class="icon-newspaper"></i></a>							
