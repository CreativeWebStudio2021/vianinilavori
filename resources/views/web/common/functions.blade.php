...
function truncateHtml($html, $maxLength) 
{
    ...
    // Imposta l'errore di caricamento silenzioso
    libxml_use_internal_errors(true);

    // Aggiungi l'intestazione UTF-8 al contenuto HTML
    $doc->loadHTML('<?php xml encoding="UTF-8">' . $html, LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD);
    libxml_clear_errors();

    $totalLength = 0;
    $truncated = false;

    // Funzione ricorsiva per attraversare e troncare i nodi
    $truncateNodes = function ($node) use (&$totalLength, $maxLength, &$truncated, &$truncateNodes) {
        if ($truncated) {
            return;
        }
        ...
        if ($node->nodeType === XML_TEXT_NODE) {
            ...
            if ($totalLength + $textLength > $maxLength) {
                $node->nodeValue = mb_substr($node->nodeValue, 0, $maxLength - $totalLength, 'UTF-8');
                $truncated = true;
            }

            $totalLength += mb_strlen($node->nodeValue, 'UTF-8');
    ...
    $truncateNodes($doc->documentElement);

    // Ritorna il contenuto come stringa HTML
