...
class Functions_adm extends Config {
	...
	public function scrivi_img($nomef,$file, $direi="")
	{
		...
		$nome = str_replace("è", "i", $nome);
		$nome = str_replace("è", "o", $nome);
		$nome = str_replace("è", "u", $nome);	
		$nome = str_replace("`", "", $nome);
	...
	public function thumbjpg( $larg ,  $file ,$filename, $dir_imm="", $start="" )
	{
		...
		if($larg>0)
		{
			...
			imagejpeg($dst_img, $dir_pic."/".$start_file.$filename, 100);
		}		
	}
	...
	/*
	***********************************************
	** Questa è la funziona che fa l'inserimento **
	***********************************************

	E' importante che al nome dei campi corrisponda il nome delle colonne nella TABELLA DEL DB
	$arr_no è l'array che spiega quali campi non bisogna inserire
	$arr_thumb è l'array che  contiene i campi da trasformare in thumbnail
	$arr_thumb['nome_campo'] = "larghezza_thumb" (in pratica io controllerè che l'array contiene un qualche valore, se lo contiene, allora lo passo a thumbjpg come larghezza) 
	$tabella è la tabella
	L'unico campo obligatorio è il primo gli altri sono facoltativi
	*/
...
//		print_r($arr_no);
		...
		foreach ($_POST as $key => $value) {
			...
			if (!isset($arr_no[$key]) && $value !== "") {
				$campi['nome']   .= " $key ,";
				$campi['valore'] .= " :$key ,";
				$array_valori[':' . $key] = str_replace('\"', '"', $value);
			}
			...
			if($dim!=0 && !isset($arr_no[$nome_campo]))
			{
				...
				*/
				...
				if($finale=="jpg" || $finale=="jpeg" || $finale=="gif" || $finale=="png")
				{
					...
					/*
					questa è la zona dell'inserimento dell'immagine
					prima devo fare un controllo sulla larghezza...
					se l'immagine è + larga di $max_w_img allora la rimpicciolisco a $max_w_img
					*/
					...
					*/
					...
					else if ($larghezza_gr > $max_w_img && ($finale== "jpeg" || $finale=="jpg")) 
					{
						...
						*/
						if(is_file("$dir_img/$nome_file")) /* sicuramente è cosè, l'ho appena caricata!!! */
						{
							/* e questo è il nome del file grande appena caricato */
				...
				*/
				...
				{
					/* questo è caso in cui non è una immagine ma è un documento*/
					$nome_file = $this->scrivi_file($nome_file , $tmp_file, $dir_files);
					//echo $nome_file;
				}
					
				$campi['nome'] .= " $nome_campo ,";
				$campi['valore'] .= " :$nome_campo ,";
				$array_valori[':'.$key]=$nome_file;
			}
			next($_FILES); 
		}
		...
		//echo $query."<br/>";
		//print_r ($array_valori)."<br/>";
		$risu = $open_connection->connection->prepare($query);
		$risu->execute($array_valori);
	...
	/*
	***********************************************
	* Questa è la funziona che invece fa l'update *
	***********************************************
	*/
	...
	{		
		...
		if($num_campi>0){
			...
			/* questo è l'array con nome campi e valori del db */
			...
			for($x=0; $x<$num_file; $x++){
				...
				if($dim!=0)
				{
					...
					else /*non è un'immagine */
					{
						$nome_file = $this->scrivi_file($nome_file , $tmp_file, $dir_files);
						$dir_old = $dir_files;
					}
					
					$stringa .= $nome_campo."=:".$nome_campo." ,";
					$array_valori[':'.$nome_campo]=str_replace('\"','"',$nome_file);
	...
	***************** andrebbero rese piè generiche e funzionali ****************
	*****************************************************************************
	primo valore la tabella in cui lavorare
	secondo valore il valore del campo unico (in genere il campo è id)
	...
	*/
	...
	public function campo_ins($titolo, $campo_nome, $tipo_campo, $nome_array="no", $style="", $value="", $field = "", $dir_imm = "", $dir_fil = "")
	{
		...
		*/
			...
			<script language=javascript>
				function ".$campo_nome."_aggiorna(){
					...
					/* se uno dei due campi non è selezionato la textarea deve essere vuota*/
			...
			echo	"<div class=\"mws-form-row\">
				<label class=\"mws-form-label\">$titolo</label>
				<div class=\"mws-form-item\">";
			/* questa è la parte che gestisce i giorni */
			...
			/* questa è la parte che gestisce i mesi */
			...
			$keys_mesi = array_keys($this->mega_mesi);
				for ($x = 0; $x < $num_mesi; $x++) {
					$kiave = $keys_mesi[$x];
					$valore = $this->mega_mesi[$kiave];
					echo "<option value=\"$kiave\">$valore</option>\n";
				}

			echo "</select>&nbsp;&nbsp;";
			
			/*questa è la parte che gestisce gli anni*/
			...
			$keys_anni = array_keys($this->mega_anni);
			for ($x = 0; $x < $num_anni; $x++) {
				$kiave = $keys_anni[$x];
				$valore = $this->mega_anni[$kiave];
				echo "<option value=\"$kiave\">$valore</option>\n";
			}
		...
		*******************************************/
			...
			echo	"<div class=\"mws-form-row\">
				<label class=\"mws-form-label\">$titolo</label>
				<div class=\"mws-form-item\">";
			/* questa è la parte che gestisce le ore */
			...
			/* questa è la parte che gestisce i minuti */
	...
	public function campo_mod($titolo , $campo_nome , $value, $tipo_campo , $nome_array="no", $att_cmd, $att_rec, $style="", $field="",$dir_imm="", $dir_fil="", $no_canc=""){
		/* se il parametro $dir_fil è non null, si prende la directory indicata invece della default 'files' per caricare i files dinamici
		...
		/* stile dei campi */
		if($style)	$stile = $style;	else $stile = "";
		/* se $no_canc ha un qualunque value 'true' non è possibile cancellare la immagine o il file caricati, ma solo modificarli (sostituisce la validazione dell'obbligatorietè dei campi file che nel _mod non puè funzionare */
		...
		if($tipo_campo == 1){
			...
			<div class=\"mws-form-row\">
				<label class=\"mws-form-label\">
					...
					";
					if($value!="" && $noc==0){
						echo"<br/><a href=\"admin.php?cmd=$att_cmd&id_rec=$att_rec&campocanc=$campo_nome\" class=\"testo10\" ><img align=\"middle\" src=\"img/erasure.png\" alt=\"Cancella il contenuto del campo\"></a>";
					}
				echo "</label>
				<div class=\"mws-form-item\">
					<input name=\"$campo_nome\" type=\"text\" class=\"medium\" value=\"$value\" id=\"$fie\">
		...
		else if($tipo_campo == 4)
		{
			...
			<div class=\"mws-form-row\">
				...
				if($src)
				{
					...
					if($value!="" && $noc==0)
					{
					echo"<a href=\"admin.php?cmd=$att_cmd&id_rec=$att_rec&campocanc=$campo_nome\" class=\"testo10\"  style=\"color:#28282C\"> <i style=\"color:#29292c; font-size:1.3em\" class=\"fa fa-trash\" aria-hidden=\"true\"></i></a>
					";
					}
				}
				echo "
			...
			<div class=\"mws-form-row\">
				<label class=\"mws-form-label\">$titolo</label>
				<div class=\"mws-form-item\">
					<a href=\"$dir_files/$value\" target=\"_blank\" class=\"$stile\" style=\"color:#28282C\">Documento attuale : <b>$value</b></a>";
				if($value!="" && $noc==0){
				echo"&nbsp;&nbsp;<a href=\"admin.php?cmd=$att_cmd&id_rec=$att_rec&campocanc=$campo_nome\" class=\"testo10\" style=\"color:#28282C; text-decoration:none\">  <i style=\"color:#29292c; font-size:1.3em\" class=\"fa fa-trash\" aria-hidden=\"true\"></i></a>
				";}
				echo"<br>
				<input name=\"$campo_nome\" type=\"file\" class=\"medium\" size=\"60\" id=\"$fie\">
				</div>
			...
			<div class=\"mws-form-row\">
				...
				if($tipo_campo!='6o') echo "<option value=\"\">Seleziona</option>";
				$count_arr = count($nome_array);
				$keys = array_keys($nome_array);
					for ($x = 0; $x < $count_arr; $x++) {
						$kiave = $keys[$x];
						$valore = $nome_array[$kiave];
	...
	/* 
	Questa fondamentalmente è una revisione della funzione di inserimento campi dell'amministrazione
	...
	*/
				...
				*/
				...
				{		
					...
					*/
					...
					/* questo è il caso della prima che va rimpicciolita o che non è ne jpg ne jpeg */
					...
					/* questo è il caso che larghezza supera i $max_w_img e la foto è un jpg */
					else if ($larghezza_gr > $max_w_img && ($finale== "jpeg" || $finale=="jpg")) 
					{
						/*
						què devo creare l'immagine piccola
						prima devo decidere il nome
						*/
						if(is_file("$dir_img/$nome_file"))
						{
	...
	public function to_htaccess_url($str_in, $sito, $subdir="",$len="",$words="")
	{
		...
		$str_out = str_replace("è", "e", $str_out);
		$str_out = str_replace("è", "a", $str_out);
		$str_out = str_replace("è", "i", $str_out);
		$str_out = str_replace("è", "o", $str_out);
		$str_out = str_replace("è", "u", $str_out);	
		...
		*/
		...
		quindi va troncata ad un valore di default (in genere puè essere un txt fino a 250 caratteri) 
		*/
		{
	...
	Non raddoppia http:// per i link giè di secondo livello
	*/
	...
	*/
	...
	{
		$str_out = str_replace("è", "e'", $str_in);
		$str_out = str_replace("è", "e'", $str_out);
		$str_out = str_replace("è", "a'", $str_out);
		$str_out = str_replace("è", "i'", $str_out);
		$str_out = str_replace("è", "o'", $str_out);
